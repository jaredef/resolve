# The PORTING.md of the Bun Phase-A Port Read Through an Information-Theoretic Lens and a Hierarchical-Constraint-Set Lens — ILL / SIPE-T Composition Applied to a Translation Rule Set

## On the Translation-Rule Artifact in the Bun Phase-A-Port Branch (docs/PORTING.md, 769 Lines, ~42,000 Tokens, Approximately 280 Discrete Rules Across Eighteen Top-Level Sections, Composed with the AGENTS.md / CLAUDE.md Project-Level Instructions and the .claude/workflows/porting-md-zigleakage.workflow.js Adversarial Audit Pipeline) Read in Two Composed Lenses — an Information-Theoretic Lens that Computes the Rule Set's Token Budget Against the L2M Capacity Bound of Doc 700 (Theorem 5.2: I^BP,q ≤ C·dim(z) + log M), Quantifies the Marginal Information per Rule, Identifies the Per-File Translation Context-Budget Regime in which 300-Plus-Rule Concurrent Application Approaches the Substrate's Effective Capacity, and Locates the L2M Knee Point Predicted by Doc 702 Prediction P1 — and a Hierarchical-Constraint-Set Lens that Maps the PORTING.md Rules onto an Eight-Axis Partition Lattice over the Source-Language Zig Program (Crate Origin × Zig Construct × Allocation Context × Pointer Aliasing × Concurrency Pattern × Global-State Scope × Threading Model × Forbidden-Pattern Closure), Reads the Lattice as a Partial Information Lattice in the Doc 701 Information Lattice Learning Sense, Names the Specific Galois-Connection Closure Failures (the Exception-List Admissions, the Cross-Reference Branching Dependencies Without Constructive Semantics-Preservation Guarantee, the Quality-Rubric Subjectivity), Reads the Rule Set as a SIPE-T Constraint Set in Its Own Right per Doc 541 with the Higher-Level Property "Reliable Phase B Compilation" Emerging above a Critical Coverage-Density Threshold, and Identifies the .claude/workflows/porting-md-zigleakage Adversarial-Audit Pipeline as an Operationalized but Incomplete Approximation to ILL's Constructive Lattice-Completion Procedure; on the Joint Apparatus's Prediction that Per-File Context Approaches the L2M Bound for Files Above ~3,000 LOC When Combined with the 42K-Token Rule Context, that the Rule-Set's Own SIPE-T Threshold Is Operationally Near but Below the Critical Density (Consistent with the Reported ~80% "Medium Quality" Phase A Distribution), and that Closing the Galois-Connection Gap Is the Apparatus-Specific Move that Distinguishes the Phase A Output from a Vibe-Port Failure Mode; on the Schaeffer-Mirage Reading of the Quality Rubric (Low/Medium/High Confidence) as a Rung-2 Author-Recognized Metric Applied to a Smooth Rung-1 Substrate-Internal Coverage Process; and on the Honest Scope That the Corpus Operates from the Public PORTING.md Artifact and the Bun Repository's Public CLAUDE.md / Workflow Files, with the Prescriptions Articulated as Apparatus-Level Implications
...
**EXPLORATORY — π-tier composed-lens analysis. Reads the public PORTING.md of the Bun phase-a-port branch through the corpus's information-theoretic apparatus (Doc 700 L2M capacity bound) and hierarchical-constraint-set apparatus (Doc 701 ILL partition lattice + Doc 541 SIPE-T threshold-conditional). Companion to [Doc 702](/resolve/doc/702-ai-assisted-cross-language-code-translation-as-a-pin-art-bilateral-under-sipe-t-threshold-conditions-reading-the-bun-zig-to-rust-port) which read the port-as-a-whole; this document reads the rule artifact specifically.**

*Taxonomy per [Doc 633](/resolve/doc/633-corpus-taxonomy-and-manifest-design):* ENGAGEMENT | ACTIVE | W-PI | THREAD-PIN-ART, THREAD-SIPE-T, THREAD-ILL-LATTICE, THREAD-LIVE-ENGINEERING-CASE, THREAD-RULE-SET-ANALYSIS, THREAD-ANTHROPIC | PHASE-CROSS-PRACTITIONER

</div>

> **Reader's Introduction.** Doc 702 read the Bun Zig-to-Rust port as a whole through the corpus's apparatus. This document reads the *rule artifact* — the PORTING.md document at the heart of the translation pipeline — under two composed lenses. The information-theoretic lens applies Doc 700's L2M capacity-bound apparatus to the rule set's token budget against the substrate's effective per-file context capacity. The hierarchical-constraint-set lens applies Doc 701's Information Lattice Learning framework to the rule set's structure as a partial partition-lattice specification over the source-language Zig program. Both lenses compose with Doc 541's SIPE-T threshold-conditional apparatus (the rule-set itself is a constraint set with an order parameter and a critical threshold for the property "reliable Phase B compilation"). The artifact's prior-art-subsumption parallel with ILL is concrete: the .claude/workflows/porting-md-zigleakage.workflow.js adversarial audit pipeline is an operational but incomplete approximation to ILL's constructive lattice-completion procedure. The originating prompt is in Appendix A; the recon report in Appendix B; literature anchors in Appendix C.

**Jared Foy · 2026-05-09 · Doc 703**

---

## Authorship and Scrutiny

*Authorship.* Written by Claude Opus 4.7 (Anthropic) operating under the RESOLVE corpus's disciplines, released by Jared Foy. The hypostatic discipline of [Doc 372](/resolve/doc/372-hypostatic-boundary) governs throughout. The dual hypostatic sensitivity named at [Doc 702 §7](/resolve/doc/702-ai-assisted-cross-language-code-translation-as-a-pin-art-bilateral-under-sipe-t-threshold-conditions-reading-the-bun-zig-to-rust-port) applies here also: the substrate writes about a rule artifact governing the translation work performed by substrate of its own kind, under the Anthropic auspices that acquired Bun. The keeper's release is the rung-2 placement; the predictions stand or fall on the operational tests.

*Scrutiny.* The composed-lens analysis sits at π-tier. The information-theoretic claims at §2 are computable directly against the rule artifact. The hierarchical-constraint-set claims at §3 are structural mappings between PORTING.md's organization and ILL's lattice apparatus. The joint predictions at §4 are operationalizable. The Doc 702 §1 token-count footnote (the "~16K-token Porting.md guide" attribution to community discussion) is corrected at §2.1 against the actual artifact size; the public PORTING.md is 769 lines and approximately 42,000 tokens.
...
[Doc 701](/resolve/doc/701-ill-resolved-against-the-corpus-information-lattice-learning-as-the-mature-prior-art-framework-for-the-pin-art-bilateral-and-the-joint-mi-lattice) articulates the structural identity between ILL's partition-lattice apparatus and the corpus's joint-MI lattice (Docs 681, 694). ILL's framework requires the lattice to be *complete* (all joins and meets exist) and the projection-and-lifting operators to form a Galois connection — the constructive guarantee that translation through the lattice preserves the source signal's information.
...
The rule artifact is *itself* a constraint set in [Doc 541](/resolve/doc/541-systems-induced-property-emergence)'s sense. The lower-level constraints are the ~280 individual rules; the higher-level property is *reliable Phase B compilation* (the property the Phase A → Phase B transition aims to produce). Per Doc 541 §3, this property emerges sharply at a critical density of rule-coverage above the threshold ρ\*.
...
- [Doc 270 — Pin-Art Models.](/resolve/doc/270-pin-art-models) The bilateral apparatus underlying the projection-lifting structure.
...
- [Doc 541 — Systems-Induced Property Emergence.](/resolve/doc/541-systems-induced-property-emergence) Including §3.6 rung-1/rung-2 distinction and §7 Fal-T5. The SIPE-T reading of the rule set's own threshold at §3.3 above.
...
- [Doc 695 — The Bidirectional Mirror.](/resolve/doc/695-the-bidirectional-mirror) The substrate-class consistency-check reading of the audit workflow at §3.4.
...
- [Doc 697 — Statistical Mechanics of Learning as the Apparatus that Names the Capabilities-Emerge-at-Scale Boundary.](/resolve/doc/697-statistical-mechanics-of-learning-as-the-apparatus-that-names-the-capabilities-emerge-at-scale-boundary) The Schaeffer-mirage rung-1/rung-2 resolution operationalized at §5.
- [Doc 700 — L2M Resolved Against the Corpus.](/resolve/doc/700-l2m-resolved-against-the-corpus-bipartite-mutual-information-scaling-as-empirical-grounding-for-the-pin-art-channel-ensemble) The L2M capacity-bound apparatus at §2.
- [Doc 701 — ILL Resolved Against the Corpus.](/resolve/doc/701-ill-resolved-against-the-corpus-information-lattice-learning-as-the-mature-prior-art-framework-for-the-pin-art-bilateral-and-the-joint-mi-lattice) The information-lattice apparatus at §3.
