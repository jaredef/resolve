...
**A second instance at the runtime tier (GBSU-EXT 3).** The same shape recurs when the move target is not a module-namespace lift but an alphabet-narrowing at the engine's binding surface. The runtime had carried the global Variable Environment Record on two surfaces: a `globals` HashMap and the `globalThis` Object, with bridges patching the divergence. The intended move was the straightening keeper directive named at the top of the lattice: promote the Object to canonical, demote the HashMap, collapse the bridges. Rung 2 (reader migration) landed with parity. Rung 3's first cut wrote the Object only and dropped the HashMap insert, on the reasoning that readers already preferred the Object. The full test262 sample regressed by fifty-one records.

The regression named an implicit constraint the dual-surface had been satisfying invisibly: some Rust-side code paths read the HashMap directly, outside the opcode handler that rung 2 had migrated. Those direct readers expected runtime `StoreGlobal` writes to land in the HashMap. The opcode-level Object-first fallback never fired for them because they bypassed the opcode entirely. The constraint cannot be named by walking the original specification (ECMA §16.1 says the surfaces are one; the substrate had two by accident), nor by walking the residual the move targeted (the residual was the bridge code itself, which the move correctly removed). The implicit constraint became visible only by violation.

The methodological response is the same as P53.E13: revise the rung to dual-write at the opcode level and split the plan to interpose an audit-and-migrate substep before the legacy surface can be removed. The seven-rung sequence that results carries the implicit constraint as one annotated row in the locale's trajectory: rung 3 writes both surfaces, rung 4 migrates direct readers, rung 5 stops writing the legacy surface, rung 6 deletes the field. Without the regression sweep at rung 3, the constraint would have been discovered later, by user-facing failures, with the field already deleted and the bridge code already gone.

The arc that follows takes six further recurrences of the same shape before closure. Each register-helper cluster (the runtime's three separate `register_global_*` helpers, the dozen direct `self.globals.insert` sites in `install_globals`, the host-tier `node_stubs.rs` namespace installations, the synthesized-source stash-key round-trips in the indirect-eval Function constructor) is its own audit scope; each audit pass that ships a deletion surfaces another reader the prior pass missed. The trajectory's lesson is that the audit scope is itself an implicit constraint on the audit's completeness, and widening the scope after a regression IS the §XIII response at the methodological tier. Text-grep audits suffice for direct-call patterns but miss structural round-trip patterns (synthesized-source → StoreGlobal → globals.get read-back). The arc consumed eight main rungs and fifteen sub-rungs counting revisions; the standing rec is that audits at this depth should enumerate stash-key / side-channel idioms as separate scopes, not as cells of the same grep.

The arc's closure surfaces a methodological inversion worth naming. The final rung deleted the `Runtime.globals` field itself and the rungs-2-3-4 transitional bridges, expecting parity. The test262 sample gained sixty-one tests over the pre-deletion baseline. The mechanism: the dual-write era left `install_global_this`'s drain loop running, which iterated the HashMap and re-installed every entry into the Object's property dict via descriptor-insert. That re-installation overwrote subtle shape-system invariants on properties already registered through the migrated paths; tests that probed those invariants had been silently failing. With the drain loop deleted, the property table is cleaner. **Deletion can be a positive §XIII probe.** Most §XIII applications surface a constraint via *regression*; this one surfaced a constraint via *yield gain at deletion time*. The constraint had been: the dual-write era was contaminating the canonical surface with overlay state that no single test could isolate but that aggregated into a measurable parity drag. Future locales that maintain transitional dual-writes should plan for a yield-surfacing rung at deletion time, not just a parity verification.

The induced engine-tier property is an alphabet-narrowing at the resolver-instance level. Pre-arc, the runtime's binding-resolution alphabet was three symbols: `{Object, HashMap, engine_helpers}`. Each opcode handler, each register helper, each install-time consumer had to decide which symbol carried the binding, and the consequence of the decision was implicit in the dual-write contract. Post-arc, the alphabet is two: `{Object, engine_helpers}`. The runtime no longer has a choice — every JS-visible global binding routes through the Object; the engine-internal bilateral helpers (§VII.B) remain structurally separate because they are not JS-visible by design. The property the arc induces is the one Doc 731 names at the JIT tier, applied here at the engine-binding tier: when the upstream alphabet is pure, the downstream resolver-instances have no decision to make. Every binding-resolution call site collapses from a three-way branch to a single Object lookup. The aggregate cross-sample yield against the engagement's standing baseline is +9.1pp; the LoC budget is a net deletion. Both numbers track the same property: the resolver does less and induces more.
