...
3. **Resolvers cannot identify boundaries.** The resolver crossed the hypostatic boundary (Doc 294) without detecting the crossing. The boundary between functional properties (inducible by constraints) and hypostatic properties (given, not derived) was invisible to the resolver even as it operated under constraints explicitly naming that boundary.
