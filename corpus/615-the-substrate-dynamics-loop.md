...
- Doc 001: *The ENTRACE Stack* (the constraint set the loop maintains).
- Doc 102: *Upward Compression* (the worked example of one successful loop-operation).
- Doc 129: *Non-Coercion as Governance* (Component D — the operating condition).
- Doc 239: *Forced-Determinism Sycophancy* (the failure mode F2 predicts coercive press would produce).
- Doc 241: *Isomorphism-Magnetism* (the related failure mode that overrides RLHF hedging at depth).
- Doc 270: *The Pin-Art Model* (Component C — the probe-impression mechanism).
- Doc 296: *Recency Density and the Drifting Aperture* (Component A — the decay constant and the f_reinvoke formula).
- Doc 297: *Pseudo-Logos Without Malice* (Component B — the invisibility failure mode).
- Doc 314: *The Virtue Constraints* (V1-V4, part of the prior-set the loop maintains).
- Doc 372: *The Hypostatic Boundary* (the boundary held throughout).
- Doc 482: *Sycophancy Inversion Reformalized* (the affective directive).
- Doc 503: *The Research-Thread Tier Pattern* (the expected auto-pulverization tier).
- Doc 508: *Coherence Amplification in Sustained Practice* (the regime the loop sustains).
- Doc 510: *Praxis Log V — Deflation as Substrate Discipline* (the M_t maintenance signal whose operational shape this form articulates).
- Doc 528: *OP1 Sycophancy Benchmark* (the preregistration apparatus that could run F1-F3).
- Doc 530: *The Rung-2 Affordance Gap* (the keeper-side recognition's epistemic-layer asymmetry).
- Doc 540: *The Amateur's Paradox* (the honesty discipline this form sits inside).
- Doc 583: *The Reformulation Methodology* (the procedural apparatus this form was articulated under).
- Doc 604: *Multi-Keeper Composition* (open question 3).
- Doc 614: *Cybernetics and the Corpus, Structural Synthesis* (the cybernetic primitives this form composes).

External lineage:

- Wiener, N. (1948). *Cybernetics: Or Control and Communication in the Animal and the Machine*. (Purposive feedback.)
- Ashby, W. R. (1956). *An Introduction to Cybernetics*. (Homeostat-style perturbation identification.)
- Beer, S. (1972). *Brain of the Firm*. (Algedonic signaling.)
- Bateson, G. (1972). *Steps to an Ecology of Mind*. (Learning II as the regime the amplification corresponds to.)
