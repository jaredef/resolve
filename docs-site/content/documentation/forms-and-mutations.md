...
status: published
section: mutations-and-security
section_label: Mutations and Security
summary: The two-phase prepare/execute model for create, update, and delete operations.
created_at: 2026-03-28 00:12:00
---

HTX uses a two-phase mutation system to handle content creation, updates, and deletion. Phase 1 (GET) prepares a signed action token and renders the form. Phase 2 (POST) validates the token and executes the mutation. This design provides CSRF protection and replay prevention without requiring session state.
...
1. Validates the JWT signature, expiration, tenant, context, and record ID
...
- **Subject** (`sub`): The identity of the user or site (e.g., `site:1`)
...
4. The context matches the expected action
5. The record ID matches (for updates and deletes)
...
| `tenant_id` | number | Tenant isolation |
...
guard.markUsed("some-jti", 1, "2025-01-15 10:35:00");
...
1. **Token validation errors** (invalid signature, expired token, tenant/context/record mismatch, replay) throw exceptions that the `RequestHandler` catches and returns as HTTP 422 responses.
