...
3. **Context match** -- The `htx-context` claim must match the expected content type.
4. **Record ID match** -- When an expected record ID is provided, the `htx-recordId` claim must match.
...
guard.markUsed("some-jti", "2026-03-28 12:05:00");
...
Because the token is scoped to a specific content type and record, and because it expires after a short window, a cross-site attacker cannot forge a valid payload. The token acts as both a CSRF token and a mutation authorization proof.
