...
This is passed through to the adapter as a string. Whether the adapter uses it for projection (selecting only those columns) is an implementation detail. The SQLite reference adapter does not currently filter columns based on this value -- it always returns all fields.
